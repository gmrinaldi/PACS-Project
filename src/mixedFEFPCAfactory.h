...
class MixedFEFPCAfactory
{
	...
	static std::unique_ptr<MixedFEFPCABase<ORDER,  mydim,  ndim>> createFPCAsolver(const std::string &validation, const MeshHandler<ORDER,mydim,ndim>& mesh, const FPCAData<ndim>& fpcaData){

	if(validation=="GCV")
	    return make_unique<MixedFEFPCAGCV<ORDER,  mydim, ndim>>(mesh,fpcaData);

	else if(validation=="KFold")
	    return make_unique<MixedFEFPCAKFold<ORDER,  mydim, ndim>>(mesh,fpcaData);

	else if(validation=="NoValidation")
	    return make_unique<MixedFEFPCA<ORDER,  mydim, ndim>>(mesh,fpcaData);
